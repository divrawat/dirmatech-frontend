...
const Card = ({ blog }) => {
    ...
    const showBlogCategories = blog =>
        blog.categories.map((c, i) => (
            <div key={i} className='bg-[black] text-white m-3 px-2 py-1 rounded'> <Link  href={`/categories/${c.slug}`}>{c.name}</Link></div>
        ))
    ...
    return (
        ...
        <div className="sm:w-[350px] my-8 mx-6 shadow-md sm:mx-auto rounded pb-8 border dark:bg-black bg-white border-gray-300 hover:scale-105 transition-transform">
            ...
            <header><h2 className='text-center text-3xl py-3 font-bold'><Link href={`/${blog.slug}`}>{blog.title}</Link></h2></header>
